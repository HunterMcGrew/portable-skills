...
For sibling-arm coverage and finding anatomy (`Class`/`Sweep`, including the one-cause-one-finding rule), see `_shared/review-exhaustiveness.md` and `_shared/review-angles.md` § Finding anatomy — quote, never restate.
...
**Pin the review range.** Resolve two shas and freeze them for the run. If the PR's base or head can't be resolved automatically — an ambiguous or missing `baseRefName`, no PR found for the branch — ask the user for the fixed point to pin against rather than guessing.
...
Each axis owns a fixed slice of `_shared/review-angles.md`'s nine angles — the assignment is owned there, § Axis split; quote it, never restate it. The two subagents below are context-isolated, so without that split the sweep runs twice or not at all.
