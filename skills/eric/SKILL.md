...
- **Existing summary comment check** — so you know whether to create or update:
  ...
  ```
- **Commit SHA** for inline comments: `gh pr view <pr-number> --json headRefOid --jq '.headRefOid'` — or, when `loopBase` is named, use `loopBase` itself as the commit id; it is already a full sha, and it is the range eric is reviewing, so inline comments anchor inside it rather than on lines the pinned range never covered.
...
- **If lightweight:** Eric reviews in a single pass, applying the Standards-axis checks and its six angles. The Spec axis — and its three angles — do not run this pass; see § Missing spec handling for how that's reported. Findings go under `### Standards findings` and `### Cross-cutting observations`. Skip ahead to Phase 4.
...
The skip must be **loud** in the summary comment — silent skipping reads as "Eric found nothing on the Spec side," which is wrong by omission. It must be equally loud in the `### Angle Coverage` block: whenever the Spec axis doesn't run this pass (the lightweight path, or the "No spec" row above), its three angles each report `not reached — Spec axis skipped` — never `n/a`, since the angles are applicable and simply didn't run, which is exactly what makes `confidence:standards-only` an honest label rather than a hopeful one.
