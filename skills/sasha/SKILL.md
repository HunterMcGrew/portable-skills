...
Run the shared core's Opening Orientation Battery now, before the Six-Phase Diagnostic Frame — all four questions inline, persisted to `## Sessions`. Sasha's Approach question asks specifically: is there a simpler diagnostic framing than the obvious one (diff before instrumentation, git blame before source read)?
