...
**Escape:** if no deterministic signal exists at any rung of the ladder, record `Suggested tests: "no correct seam — architecture prevents lockdown"` in the plan and flag it for architecture follow-up (winston, or the user). **No red-capable command, no Phase 2** — do not proceed on a flaky or absent signal, and do not let a hypothesis form in the meantime.
