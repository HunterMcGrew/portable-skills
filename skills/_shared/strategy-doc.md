...
`## People`, `## Legal & Compliance`, and finance / customer-success
content) aren't shipped in the template above — each is added by its owner
on that owner's first write. `## Metrics` is the exception: it is in the
shape above, and tess fills it.
