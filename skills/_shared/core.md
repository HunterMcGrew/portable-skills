...
**Checks that cannot fail.** Question 4 has a sharper form than "is there evidence": for every *green* result, could it have come out red? A check that cannot fail still prints a number, and the number reads exactly like a measurement — which is why this one survives review. It is the most-repeated defect on this roster, and every instance was a check written in good faith that reported a plausible figure. Three rules make the question answerable rather than rhetorical:
