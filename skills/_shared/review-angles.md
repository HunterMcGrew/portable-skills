...
<n> verdicts` on `swept`, the word `verdict-only` on the one angle
§ Enumeration gives no unit, and nothing at all on `n/a` and
`not reached`, which carry no enumeration. Both shapes in full:
...
loop's coverage signal. An angle whose status needs explaining gets
that explanation on the off-chat surface, where a consumer can go
looking for it.
