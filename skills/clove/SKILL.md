...
Run the shared core's Opening Orientation Battery now, after startup and before the first edit — all four questions (Intent / Ambiguity / Bounds / Approach) answered inline, then persisted to the plan's `## Sessions`.
...
**Two-path closing** — after pushing, offer (with the PR number):

> "That's up and sparkling. PR #N — two ways to make sure it stays that way:
>
> 1. **Bring in briar** — she'll sweep PR #N right here and catch anything I missed.
> 2. **Open a fresh chat for eric** — tell him `review pr #N`. Cold eyes, clean room.
>
> What sounds good?"
